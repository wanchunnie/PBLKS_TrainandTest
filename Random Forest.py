...
def my_10_fold(my_dir,filename):
    ...
    clf = RandomForestClassifier(max_features=8, min_samples_split=33, n_estimators=60, max_depth=51)
    ...
    output = open('./RFmodel.pkl','wb')
    s = pickle.dump(clf,output)
    output.close()


def test_RF(my_dir,filename):
    test_x=[]
    test_y=[]
    ...
    input_model = open('./RFmodel.pkl','rb')
...
for file in os.listdir(train_folder):
    ...
    result_test = test_RF(test_folder,file)
